...
#let data = (
  ...
  "辟": (
    ...
    "zi-list": (
      ...
      ("辟", "必益", "幫昔入三開", ``` pi_E_sk ```, "錫", ``` peg ```, []),
      ("璧", "必益", "幫昔入三開", ``` pi_E_sk ```, "錫", ``` peg ```, []),
      ("躄", "必益", "幫昔入三開", ``` pi_E_sk ```, "錫", ``` peg ```, []),
      ("𠪮", "必益", "幫昔入三開", ``` pi_E_sk ```, "錫", ``` peg ```, []),
      ...
      ("辟", "房益", "並昔入三開", ``` bi_E_sk ```, "錫", ``` beg ```, [#xs(``` s ```)爲使動前綴，“辟（避）”的自動形式爲#xs(``` beg ```)，使動詞#xs(``` s.beg ```)>#xs(``` peg ```)。]),
  ...
  "扁": (
    ...
    "zi-list": (
      ...
      ("猵", "毗忍", "並真上A開", ``` bin ```, "真n", ``` bin? ```, []),
      ("㻞", "府巾", "幫真平B開", ``` p1in ```, "真n", ``` p_r_tin ```, []),
    )
  ),
  "𡬯": (
    "pinyin": [bia3n],
    "zi-list": (
      ("𡬯", "方斂", "幫鹽上B開", ``` p1_E_sm ```, "談o", ``` p_r_tom? ```, [原爲山韻，但是根據諧聲系統應該爲刪韻，《切韻》音系二等有刪、山的歷史層次。]),
    )
  ),
  "釆": (
    "pinyin": [bia4n],
    "zi-list": (
      ...
      ("釆", "蒲莧", "並刪去二開", ``` b_M_t{n ```, "元a", ``` b_r_ta_-ns ```, []),
      ("皤", "博木", "幫歌平一合", ``` p_wA ```, "歌a", ``` pa_-l ```, []),
      ("嶓", "博木", "幫歌平一合", ``` p_wA ```, "歌a", ``` pa_-l ```, []),
      ("番", "博木", "幫歌平一合", ``` p_wA ```, "歌a", ``` pa_-l ```, []),
